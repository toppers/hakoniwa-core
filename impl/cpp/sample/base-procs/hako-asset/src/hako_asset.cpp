...
int main(int argc, const char* argv[])
{
    ...
    hako::logger::init(asset_name);
    hako::logger::get(asset_name)->info("delta={0} usec asset_name={1}", delta_usec, asset_name);
    ...
    bool ret = hako_asset->asset_register(asset_name, callback);
    if (ret == false) {
        hako::logger::get(asset_name)->error("Can not register");
        return 1;
    }
    ...
    hako_asset->asset_unregister(asset_name);
    hako::logger::get(asset_name)->flush();
