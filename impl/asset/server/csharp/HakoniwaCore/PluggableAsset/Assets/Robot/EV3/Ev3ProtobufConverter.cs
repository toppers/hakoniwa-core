...
namespace Hakoniwa.PluggableAsset.Assets.Robot.EV3
{
    class Ev3ProtobufConverter : IInsideAssetController
    {
        ...
        {
            this.name = name;
            this.roboname = name.Split('_')[0];
            if (this.roboname == null)
            {
                throw new ArgumentException("Invalid converter name:" + name + ": must be <roboname>_<convname>");
            }
            SimpleLogger.Get().Log(Level.INFO, "ConverterName:" + name);
            SimpleLogger.Get().Log(Level.INFO, "RobotName:" + this.roboname);
        }

        public void Initialize()
        {
            this.pdu_io = PduIoConnector.Get(this.GetName());
            this.in_pdu_reader = this.pdu_io.GetReader(this.roboname + "_Ev3ActuatorPdu");
            if (this.in_pdu_reader != null)
            {
                IPduReader tmp1 = this.pdu_io.GetReader(this.roboname + "_Ev3ActuatorProtobufPdu");
            ...
            this.in_pdu_writer = this.pdu_io.GetWriter(this.roboname + "_Ev3SensorPdu");
            if (this.in_pdu_writer != null)
            {
                IPduWriter tmp2 = this.pdu_io.GetWriter(this.roboname + "_Ev3SensorProtobufPdu");
