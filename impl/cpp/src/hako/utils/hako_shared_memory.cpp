...
int32_t hako::utils::HakoSharedMemory::create_memory(int32_t id, int32_t size)
{
    ...
    int32_t shmid = shmget(id, total_size, IPC_CREAT | S_IRUSR | S_IWUSR);
    if (shmid < 0) {
    ...
    info.addr = metap;
    info.sem_id = sem_id;
    this->shared_memory_map_.insert(std::make_pair(shmid, info));
    //printf("segid=%d memp=%p\n", shmid, this->shared_memory_map_[shmid].addr);
    return shmid;
}

void* hako::utils::HakoSharedMemory::load_memory(int32_t shmid)
{
    void *shared_memory = shmat(shmid, 0, 0);
    ...
    this->shared_memory_map_.insert(std::make_pair(shmid, info));
    return &this->shared_memory_map_[shmid].addr->data[0];
}

void* hako::utils::HakoSharedMemory::lock_memory(int32_t shmid)
{
    struct sembuf sop;
    sop.sem_num =  0;            // Semaphore number
    sop.sem_op  = -1;            // Semaphore operation is Lock
    sop.sem_flg =  0;            // Operation flag
    int32_t err = semop(this->shared_memory_map_[shmid].sem_id, &sop, 1);
    if (err < 0) {
...
void hako::utils::HakoSharedMemory::unlock_memory(int32_t shmid)
{ 
    struct sembuf sop;
    sop.sem_num =  0;            // Semaphore number
    sop.sem_op  =  1;            // Semaphore operation is Lock
...
void hako::utils::HakoSharedMemory::destroy_memory(int32_t shmid)
{
    void *addr = this->shared_memory_map_[shmid].addr;
    if (addr != nullptr) {
        (void)shmdt(addr);
        (void)shmctl (shmid, IPC_RMID, 0);
        (void)semctl(this->shared_memory_map_[shmid].sem_id, 1, IPC_RMID, NULL);
        this->shared_memory_map_.erase(shmid);
    }
    return;
}